...
class Tensor {
 public:
  ...
  typename TTypes<T>::Vec vec() {
    return tensor<T, 1>(); // 前面是类型，后面是维数的个数
  }

  template <typename T>
  typename TTypes<T>::Matrix matrix() { // 返回的是可写的二维矩阵
    return tensor<T, 2>(); // 前面是类型，后面是维数的个数
  }
  ...
  typename TTypes<T>::Flat flat() {
    return shaped<T, 1>({NumElements()}); // 返回重塑之后的一维向量
  }
  ...
  template <typename T>
  typename TTypes<T>::Scalar scalar(); //标量

  /// Const versions of all the methods above.
  template <typename T>
  typename TTypes<T>::ConstVec vec() const { // 一维向量
    return tensor<T, 1>();
  }

  template <typename T>
  typename TTypes<T>::ConstMatrix matrix() const { //二维矩阵，返回的是常矩阵
    return tensor<T, 2>();
  }
  ...
  template <typename T>
  typename TTypes<T>::ConstScalar scalar() const; // 常标量
