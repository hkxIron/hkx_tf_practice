...
def export_state_tuples(state_tuples, name):
  for state_tuple in state_tuples:
    tf.add_to_collection(name, state_tuple.c) # cell state
    tf.add_to_collection(name, state_tuple.h) # hidden state


def import_state_tuples(state_tuples, name, num_replicas):
  restored = []
  for i in range(len(state_tuples) * num_replicas):
    c = tf.get_collection_ref(name)[2 * i + 0] # cell, get_collection_ref返回一个list
    h = tf.get_collection_ref(name)[2 * i + 1] # hidden
