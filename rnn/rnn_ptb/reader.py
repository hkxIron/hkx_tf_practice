...
def _build_vocab(filename):
  ...
  counter = collections.Counter(data) # Counter({'b': 3, 'c': 2, 'a': 1})
  count_pairs = sorted(counter.items(), key=lambda x: (-x[1], x[0])) # 按count降序排序

  words, _ = list(zip(*count_pairs))
  word_to_id = dict(zip(words, range(len(words)))) # words只是一个元组: ('a','b','c')

  return word_to_id # {'b': 0, 'c': 1, 'a': 2}
...
def ptb_producer(raw_data, batch_size, num_steps, name=None):
  ...
  with tf.name_scope(name, "PTBProducer", [raw_data, batch_size, num_steps]):
    ...
    print("before convert:",epoch_size)
