...
file_name = 'D:\\my_data\\dataset\\tiny-shakespeare\\tiny-shakespeare.txt'
...
def build_basic_rnn_graph_with_list(
        ...
        num_classes=num_classes, # 即vocab_size
    ...
    rnn_input_list = [tf.squeeze(i, squeeze_dims=[1]) for i in tf.split(x_one_hot, num_steps, 1)] # 将一个tensor拆成num_step个tensor,每个tensor:[batch,num_classes]
    print("rnn_inputs:",rnn_input_list)
    '''dropout rnn inputs'''
    rnn_input_list = [tf.nn.dropout(rnn_input, keep_prob) for rnn_input in rnn_input_list]

    cell = tf.nn.rnn_cell.BasicRNNCell(hidden_size)
    init_state = cell.zero_state(batch_size, tf.float32)
    '''使用static_rnn方式'''
    rnn_outputs, final_state = tf.contrib.rnn.static_rnn(cell=cell, inputs=rnn_input_list,
                                                         initial_state=init_state)
    print("rnn_outputs:",rnn_outputs,"\nfinal_state:",final_state) # rnn_outpus: 即hidden_state list,num_step个[batch,hidden], final_state:[batch,hidden]
    '''dropout rnn outputs'''
    ...
    y_as_list = [tf.squeeze(i, squeeze_dims=[1]) for i in tf.split(y, num_steps, 1)] # 将每个时间片上的y展开
...
def build_multilayer_lstm_graph_with_dynamic_rnn(
    ...
    y = tf.placeholder(tf.int32, [batch_size, num_steps], name='y')
    keep_prob = tf.placeholder(tf.float32, name="keep_prob")
    # 注意，在此处，比static_rnn那个图多了一层 embedding
    embeddings = tf.get_variable(name='embedding_matrix', shape=[num_classes, hidden_size])
    '''这里的输入是三维的[batch_size, num_steps, hidden_size]
        - embedding_lookup(params, ids)函数是在params中查找ids的表示， 和在matrix中用array索引类似,
          这里是在二维embeddings中找二维的ids, ids每一行中的一个数对应embeddings中的一行，所以最后是[batch_size, num_steps, hidden_size]
    '''
    rnn_inputs = tf.nn.embedding_lookup(params=embeddings, ids=x)
    print("dynamic rnn_inputs:",rnn_inputs) # [batch,num_step,hidden]
    cell = tf.nn.rnn_cell.LSTMCell(num_units=hidden_size, state_is_tuple=True)
    cell = tf.nn.rnn_cell.DropoutWrapper(cell, input_keep_prob=keep_prob)
    cell = tf.nn.rnn_cell.MultiRNNCell(cells=[cell] * num_layers, state_is_tuple=True)
    cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=keep_prob) # 注意一个是input_keep_prob,一个output_keep_prob
    init_state = cell.zero_state(batch_size, dtype=tf.float32)
    '''使用dynamic_rnn方式'''
    rnn_outputs, final_state = tf.nn.dynamic_rnn(cell=cell, inputs=rnn_inputs, # dynamic不在需要将input split成多个tensor了
                                                 initial_state=init_state)
    with tf.variable_scope('softmax'):
        W = tf.get_variable('W', [hidden_size, num_classes])
        b = tf.get_variable('b', [num_classes], initializer=tf.constant_initializer(0.0))

    rnn_outputs = tf.reshape(rnn_outputs, [-1, hidden_size])  # 转成二维的矩阵
...
def build_multilayer_lstm_graph_with_scan(
    ...
    init_state = cell.zero_state(batch_size, dtype=tf.float32)
    '''使用tf.scan方式
        scan是个高阶函数，一般的计算方式是：给定一个序列[x0,x1,.....,xn]和初试状态y−1,
        根据yt=f(xt,yt−1) 计算得到最终序列[y0,y1,......,yn]
        
       - tf.transpose(rnn_inputs, [1,0,2])  是将rnn_inputs的第一个和第二个维度调换，即[num_steps,batch_size, hidden_size],
           在dynamic_rnn函数有个time_major参数，就是指定num_steps是否在第一个维度上，默认是false的,即不在第一维
       - tf.scan会将elems按照第一维拆开，所以一次就是一个step的数据（和我们static_rnn的例子类似,shape:[batch_size,hidden_size]）
       - a的结构和initializer的结构一致，所以a[1]就是对应的state，cell需要传入x和state计算
       - 每次迭代cell返回的是一个rnn_output(batch_size,hidden_size)和对应的state,num_steps之后的rnn_outputs的shape就是(num_steps, batch_size, hidden_size)
    ...
    rnn_outputs, final_states = tf.scan(fn=test_fn, elems=tf.transpose(rnn_inputs, [1, 0, 2]),
                                        initializer=(tf.zeros([batch_size, hidden_size]), init_state)
                                        )
    print("rnn_outputs:",rnn_outputs," final_states:",final_states) # output即为 last_hidden
    """
    rnn_outputs:[num_steps,batch,hidden]
    final_states: 一个list,里面有num_layers个state_tuple，每个state_tuple的结构是: (cell_state, hidden_state)
    """
    '''或者使用lambda的方式'''
    ...
    final_state = tuple([tf.nn.rnn_cell.LSTMStateTuple(
        tf.squeeze(tf.slice(c, [num_steps - 1, 0, 0], [1, batch_size, hidden_size])),
        tf.squeeze(tf.slice(h, [num_steps - 1, 0, 0], [1, batch_size, hidden_size]))) for c, h in final_states])
...
def build_final_graph(
    ...
    y = tf.placeholder(tf.int32, [batch_size, num_steps], name='y')
    keep_prob = tf.placeholder(tf.float32, name='keep_prob')
    embeddings = tf.get_variable('embedding_matrix', [num_classes, hidden_size])
    ...
    if cell_type == 'GRU':
        ...
        cell = LayerNormalizedLSTMCell(hidden_size)
    else:
        cell = tf.nn.rnn_cell.BasicRNNCell(hidden_size)
...
"""
g = build_final_graph(cell_type='LN_LSTM', hidden_size=hidden_size,
                      num_classes=num_classes,
                      batch_size=batch_size,
