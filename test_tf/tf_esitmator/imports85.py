...
def dataset(y_name="price", train_fraction=0.7):
  ...
  def decode_line(line):
    ...
    # csv_header.values().
    items = tf.decode_csv(line, record_defaults=list(defaults.values()))
  ...
  def has_no_question_marks(line):
    # 判定一条记录是否非法,即如果line中含有一个"?"，则为非法
    """Returns True if the line of text has no question marks."""
    # split the line into an array of characters
    chars = tf.string_split(line[tf.newaxis], "").values # tensorflow 还是有op，可以支持 split
    # for each character check if it is a question mark
    is_question = tf.equal(chars, "?") # 对每个字符判断是否等于 ?
    any_question = tf.reduce_any(is_question) # 在某维上计算 logical_or
    no_question = ~any_question # 对布尔tensor取反
  ...
  def in_training_set(line):
    ...
    # we are doing here.
    num_buckets = 1000000
    # 由于需要确定性的hash，所以此处我们不能用random
    bucket_id = tf.string_to_hash_bucket_fast(line, num_buckets) #  此种hash的冲突率可能较高
  ...
  base_dataset = (
      ...
      .filter(has_no_question_marks)) # 这个有点像scala的风格
  ...
  train = (base_dataset
           ...
           .cache()) # 妙啊，感觉像scala了， scala也是先构建图，然后运行
