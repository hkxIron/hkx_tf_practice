...
class Seq2SeqModel():
    ...
    def _create_rnn_cell(self):
        def single_rnn_cell():
            ...
            # 的列表中最终模型会发生错误, TODO(kexin):为啥会有这种错误
    ...
    def build_model(self):
        ...
        # 得到batch里序列长度的最大值
        self.max_target_sequence_length = tf.reduce_max(self.decoder_targets_length, axis=None, name='max_target_len')
        # decoder_targets_length:[batch]
        # mask:[batch, max_target_sequence_length]
        self.decoder_target_length_mask = tf.sequence_mask(lengths=self.decoder_targets_length,
        ...
        with tf.variable_scope('encoder'):
            ...
            # encoder_outputs用于attention，[batch,encoder_inputs_length,rnn_hidden_size]
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.beam_search:
                ...
                # encoder_ouputs_input:[batch,encoder_inputs_length,rnn_hidden_size]
                # encoder_ouputs:[batch*beam_size,encoder_inputs_length,rnn_hidden_size]
                encoder_outputs = tf.contrib.seq2seq.tile_batch(encoder_outputs, multiplier=self.beam_size)
                # 将encoder_state中的state每个也复制beam_size份
                encoder_state = nest.map_structure(lambda s: tf.contrib.seq2seq.tile_batch(s, self.beam_size), encoder_state)
                # 将encoder_inputs_length复制
                # encoder_inputs_length:[batch*beam_size]
            ...
            if self.mode == 'train':
                ...
                # helper里会定义decoder的输入
                training_helper = tf.contrib.seq2seq.TrainingHelper(inputs=decoder_inputs_embedded,
                                                                    sequence_length=self.decoder_targets_length,
                                                                    time_major=False,
                                                                    name='training_helper')
                # basic_decoder阶段会将 decoder的网络以及输入联系起来
