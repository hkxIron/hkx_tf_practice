...
class DataSet:
            ...
            if line_cnt == 0:  # csv head
                ...
                if line_cnt == 1:  # determine the value type
                    ...
                    for i in range(0, len(self.field_names)):
                        ...
                        try:
                            float(fields[i])
                            self.distinct_valueset[self.field_names[i]] = set()  #数值用set表示
                        except ValueError:
                            valueSet.add(fields[i]) #类别用字符表示
                        self.field_type[self.field_names[i]] = valueSet
                self.instances[line_cnt] = self._construct_instance(fields)  #每个instance都是一个kv字典对
            line_cnt += 1
    ...
    def _construct_instance(self, fields):
        ...
        for i in range(0, len(fields)):
            ...
            real_type_mark = self.is_real_type_field(field_name) #是否为连续数值
            if real_type_mark:
                try:
                    instance[field_name] = float(fields[i])
                    self.distinct_valueset[field_name].add(float(fields[i])) #每个连续属性下连续值
                except ValueError:
                    raise ValueError("the value is not float,conflict the value type at first detected")
            else:
                instance[field_name] = fields[i]
                self.field_type[field_name].add(fields[i]) #每个类别属性下的值统计
        return instance
    ...
    def is_real_type_field(self, name):#连续值是一个set集合,为空
        """判断特征类型是否是real type"""
