...
#
# 如果预测概率为0.8,那么应该80%是正样本, 与魏老师的想法一样,赞!!!
#
...
#y = X.target.values # x-> y
"""
np.random.seed(0)
N,D = 1000, 5
X = np.random.rand(N, D)
y = np.random.randint(low=0, high=2, size=N)
y = LabelEncoder().fit_transform(y)  #　label_encoder将字符串转为数字,便于模型处理,不过我经过实验发现不如one-hot精确
"""
...
print("loss WITH calibration : ", log_loss(ytest, ypreds, eps=1e-15, normalize=True)) # logloss会降低,auc升高
print("auc WITHOUT calibration : ", roc_auc_score(ytest, ypreds))
