...
def edx_json2srt(o): #解析json,得到字幕文件名
    i = 1
    output = ''
...
def parse_args():
    ...
    parser.add_argument('-nvt', #hkx添加
                    '--no video and subtitle',
                    dest='noVideoAndTitle',
                    action='store_true',
                    default=False,
                    help='do not download video and title')
    parser.add_argument('-ftype', #hkx添加
                    '--download the specified file,separated by "," ',
                    dest='fileType',
                    action='store',
                    default='*',
                    help='download the specified file,separated by "," ')
