// g++ -std=c++11 -o tree.o -c tree.cpp
// g++ -std=c++11 -o CommonTree CommonParentInTree.cpp tree.o &&./CommonTree
...
bool GetNodePath(const TreeNode* pRoot, const TreeNode* pNode, list<const TreeNode*>& path)
{
    ...
    {
        found = GetNodePath(*i, pNode, path); // 深度优先搜索
        ++i;
    }
...
// list里装的序列是从根结点到子节点,所以求公共结点时,不需要先求长度差
const TreeNode* GetLastCommonNode (const list<const TreeNode*>& path1,
    ...
    while(iterator1 != path1.end() && iterator2 != path2.end()) {
        if(*iterator1 == *iterator2) // 从根往下遍历,记录最后一次公共的节点
            pLast = *iterator1;
...
void Test1()
{
    ...
    ConnectTreeNodes(pNode1, pNode2); // 第一个参数为父结点,第二个参数为子结点
    ConnectTreeNodes(pNode1, pNode3); // 创建第二个孩子
    ...
    Test("Test1 p6,p8 -> p2", pNode1, pNode6, pNode8, pNode2);
    Test("Test1 p6,p5 -> p2", pNode1, pNode6, pNode5, pNode2);
}
