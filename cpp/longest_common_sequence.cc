/*
g++ -std=c++11 -o longest longest_common_sequence.cc &&./longest
*/
...
*/
// 最长公共子序列
int lcs_length(char* str1, char* str2, int **b)
{
    ...
    // 双指针的方法申请动态二维数组
    // C矩阵：[length1, length2], C矩阵用来存储子串长度
    ...
    */
    len=c[length1][length2]; // 最后一个元素即为最大值
...
void print_lcs(int **b, char *str1, int i, int j)
{
    ...
    if(b[i][j]=='\\') // 左上方
    {
        print_lcs(b, str1, i-1, j-1);//从后面开始递归，所以要先递归到子串的前面，然后从前往后开始输出子串
        printf("%c",str1[i-1]);//c[][]的第i行元素对应str1的第i-1个元素
    }
    else if(b[i][j]=='|') // 上方
        print_lcs(b, str1, i-1, j);
    else // '-'
        print_lcs(b, str1, i, j-1); // 左边
}

// 最长公共子串(注意，不是子序列,要求序是是连续的,不能断开)
int max_sub_seq(char* str1, char* str2, int& last_index) {
    int len1 = strlen(str1);
    int len2 = strlen(str2);
    int max_sub_seq_len = 0;     //记录最长公共子串长度
    const int N=20;
    ...
    for(int i = 0; i <= len1; i++) {
        for(int j = 0; j <= len2; j++) {
            if(i == 0 || j == 0) {
                ...
                if(c[i][j]>max_sub_seq_len) last_index = i-1;
                max_sub_seq_len = std::max(c[i][j], max_sub_seq_len);
...
int main(void)
{
    ...
    len=lcs_length(str1,str2,b);
    printf("str1:%s\n",str1);
    printf("str2:%s\n",str2);
    printf("最长公共子序列的长度为：%d\n",len);
    printf("最长公共子序列为：");
    print_lcs(b,str1,length1,length2);
